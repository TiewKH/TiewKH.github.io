function fillMap(selection, color, data) {
  selection
    .attr("fill", function(d) { 
        //Check if the data's ID is undefined, if undefined fill with color_na (grey), else use the colours provided by the color variable (which is derived from calcColorScale)
...
function calcColorScale(data) {
  ...
  let scale = d3.scaleQuantile()
              ...
              .range(d3.schemePurples[(quantiles_calc.length)-1]);
  //console.log(scale);
  return scale;
}
...
// Used to sort the bars by ID (in this case it would be state name)
function sortArrObj(arr,sortkey) {

  //Get the sorted array of keys
  sorted_keys = arr.map( function(elem) {return elem[sortkey];}).sort();

  //Push elements into new array based on the sorted array of keys
  newarr = [];
